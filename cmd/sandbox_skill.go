...
const sandboxSkillDir = ".claude/skills/sr-sandbox"

// installSandboxSkill writes the sr-sandbox skill under repoRoot.
func installSandboxSkill(repoRoot string) error {
	dir := filepath.Join(repoRoot, sandboxSkillDir)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("could not create sandbox skill directory: %w", err)
	}
	path := filepath.Join(dir, skillFile)
	if err := os.WriteFile(path, []byte(sandboxSkillContent), 0o644); err != nil {
		return fmt.Errorf("could not write sandbox skill: %w", err)
	}
	return nil
}

const sandboxSkillContent = `---
name: sr-sandbox
description: >
